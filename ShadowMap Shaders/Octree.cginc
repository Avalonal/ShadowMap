...
inline int DecodeNode(int4 val, out int type) {
	type = val.x >> 6;
	val.x -= type * 64;
	int ans = val.x * 16777216 + val.y * 65536 + val.z * 256 + val.w;
	return ans;
}
...
//inline float GetShadow(float3 pos, StructuredBuffer<Node> octree,int depth,int width,int height,float cell) {
//	
//	int tmpIp = 0;
//	uint x = 0, y = 0, z = 0;
